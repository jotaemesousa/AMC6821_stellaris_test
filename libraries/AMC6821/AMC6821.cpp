...
void AMC6821::readTemp11bits(uint16_t &local_temp, uint16_t &remote_temp)
{
	uint8_t lbyte = 0;
	uint8_t local_hbyte = 0;
	uint8_t remote_hbyte = 0;

	lbyte = read1(_addr, AMC6821_REG_TEMP_8THS);
	local_hbyte = getLT();
	remote_hbyte = getRT();

	local_temp = 0;
	local_temp = ((lbyte >> 5) & 0x07) | (local_hbyte << 3);
	remote_temp = 0;
...
void AMC6821::read_n(uint8_t addr, uint8_t reg, uint8_t n_bytes, uint8_t *out)
{
